...
def init_db_samples(dbmanager: Any):
    ...
    with open("inferco/database/skills/get_postgres_schema_information.py", "r") as file:
        get_postgres_schema_information_content = file.read()

    get_postgres_schema_information_skill = Skill(
        name="get_postgres_schema_information",
        description="Function to connect to a PostgreSQL database and retrieve table definitions and keys.",
        content=get_postgres_schema_information_content,
        user_id="guestuser@gmail.com",
    )
    ...
    data_engineer_assistant_config = AgentConfig(
    ...
    data_engineer_agent = Agent(
        user_id="guestuser@gmail.com", type=AgentType.assistant, config=data_engineer_assistant_config.model_dump(mode="json")
    )
    ...
    # workflows
    data_engineer_workflow = Workflow(
        name="Data Engineer Workflow", 
        description="Interacts with data sources using SQL & Python", 
        user_id="guestuser@gmail.com"
    )
    ...
    with Session(dbmanager.engine) as session:
        ...
        session.add(get_postgres_schema_information_skill)
        session.add(user_proxy)
        session.add(data_engineer_agent)
        # session.add(travel_groupchat_agent)
        # session.add(planner_assistant)
        # session.add(local_assistant)
        # session.add(language_assistant)

        session.add(data_engineer_workflow)
        # session.add(travel_workflow)
        session.commit()

        dbmanager.link(link_type="agent_model", primary_id=data_engineer_agent.id, secondary_id=gpt_4_model.id)
        dbmanager.link(link_type="agent_skill", primary_id=data_engineer_agent.id, secondary_id=generate_image_skill.id)
        dbmanager.link(link_type="agent_skill", primary_id=data_engineer_agent.id, secondary_id=get_postgres_schema_information_skill.id)
        dbmanager.link(
            link_type="workflow_agent", primary_id=data_engineer_workflow.id, secondary_id=user_proxy.id, agent_type="sender"
        )
        dbmanager.link(
            link_type="workflow_agent",
            primary_id=data_engineer_workflow.id,
            secondary_id=data_engineer_agent.id,
            agent_type="receiver",
        )
        ...
        #     agent_type="receiver",
        # )
        logger.info("Successfully initialized database with Data Engineer Workflow") # and Travel Planning
