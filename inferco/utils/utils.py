...
from ..datamodel import CodeExecutionConfigTypes, DataSource, DataSourceType, Model, Skill
...
def get_app_root() -> str:
    ...
    app_root = os.environ.get("inferco_APPDIR") or default_app_root
    return app_root
...
def get_db_uri(app_root: str) -> str:
    ...
    db_uri = os.environ.get("inferco_DATABASE_URI") or db_uri
...
def test_database_connection(data_source: DataSource):
    """
    Test the data source by establishing a connection and returning the response.
    """
    
    if data_source.type != DataSourceType.postgres.value:
        return True
    ...
    try:
        ...
        if result[0] == 1:
            print(f"Successfully pinged the database '{data_source.id=}'")
        ...
        print(f"Failed to connect to the database '{data_source.id=}'")
        return False
