...
from inferco.utils.utils import test_database_connection

from ..chatmanager import AutoGenChatManager, WebSocketConnectionManager
from ..database import workflow_from_id
from ..database.dbmanager import DBManager
from ..datamodel import Agent, Message, Model, Response, Session, Skill, Workflow, DataSource
...
async def list_data_sources(user_id: str):
    ...
    filters = {"user_id": user_id}
    return list_entity(DataSource, filters=filters)


@api.post("/data-sources")
async def create_data_sources(data_source: DataSource):
    """Create a new data source"""
    filters = {"user_id": data_source.user_id}
    return create_entity(data_source, DataSource, filters=filters)


@api.delete("/data-sources/delete")
async def delete_data_sources(data_source_id: int, user_id: str):
    """Delete a data source"""
    filters = {"id": data_source_id, "user_id": user_id}
    return delete_entity(DataSource, filters=filters)


@api.post("/data-sources/test")
async def test_data_sources(data_source: DataSource):
    """Test connection with a data source"""

    status = False
    message = "Data source connection failed"
    try: 
        if test_database_connection(data_source):
            status = True
