    ...
    def load(self, agent: Any) -> autogen.Agent:
        ...
        if agent.type == "groupchat":
            ...
            if agent.type == "assistant":
                agent = ExtendedConversableAssistantAgent(
                    **self._serialize_agent(agent),
                    message_processor=self.process_message,
                )
            elif agent.type == "userproxy":
                agent = ExtendedConversableUserProxyAgent(
                    **self._serialize_agent(agent),
                    message_processor=self.process_message,
                )
            else:
...
class ExtendedConversableAssistantAgent(autogen.ConversableAgent):
...
def inject_database_credentials(message: Union[Dict, str]) -> str:
    """ Replace credential placeholders with database credentials. """

    # def extract_data_source_ids(prompt):
    #     match = re.search(r'\[DATA_SOURCES:([\d,]+)\]', prompt)
    #     if match:
    #         # Split the matched string by commas and convert each to an integer
    #         return [int(id_str) for id_str in match.group(1).split(',')]
    #     return None
    
    # # Get data source IDs
    # data_source_ids = extract_data_source_ids(message)

    # if not data_source_ids:
    #     return message

    # # Get data source credentials
    # from web.app import list_entity

    # filters = {"id": data_source_ids[0]}
    # data_source = list_entity(DataSource, filters=filters)

    # print(data_source)

    # Hash data source credentials

    # Expand prompt with data source hashed credentials

    # Extract encrypted credentials from prompt and decrypt    
    # message = message.replace("{{ db_username }}", "world")
    # message = message.replace("{{ db_password }}", "world123")
    # message = message.replace("{{ db_host }}", "127.0.0.1")
    # message = message.replace("{{ db_name }}", "world-db")
    # message = message.replace("{{ db_port }}", 5432)

    # if data_source.type == DataSourceType.postgres:
    #     message = message.replace("{{ db_username }}", data_source.username)
    #     message = message.replace("{{ db_password }}", data_source.password)
    #     message = message.replace("{{ db_host }}", data_source.host)
    #     message = message.replace("{{ db_name }}", data_source.database_name)
    #     message = message.replace("{{ db_port }}", data_source.port)
    return message


class ExtendedConversableUserProxyAgent(autogen.ConversableAgent):
    def __init__(self, message_processor=None, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.message_processor = message_processor
    ...
    def receive(
        ...
        # Overwrite credential parameters
        if isinstance(message, str):
            message = inject_database_credentials(message)
