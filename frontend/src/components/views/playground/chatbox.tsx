...
import { cibMysql, cibPostgresql } from '@coreui/icons';
...
const ChatBox = ({
  ...
  const [dataSourceOptions, setDataSourceOptions] = React.useState<Array<IDataSourceConfig>>([]);
  ...
  const handleDataSourceToggle = (dataSource: IDataSourceConfig) => {
    setSelectedDataSources(prev => ({
      ...prev,
      [dataSource.id]: !prev[dataSource.id]
    }));
    setShowDropdown(false);
  };

  const isAnyDataSourceSelected = Object.values(selectedDataSources).some(Boolean);
  ...
  const fetchDataSourceOptions = async () => {
    const fetchUrl = `${serverUrl}/data-sources/?user_id=${user?.email}`;
    const payLoad = {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
    ...
    const onSuccess = (data: any) => {
      if (data && data.status) {
        ...
        if (data.data && data.data.length > 0) {
          setDataSourceOptions(data.data)
        }
      } else {
        message.error(data.message);
      }
          ...
          >
            ...
            >
              <textarea
                ...
                className="flex items-center w-full resize-none text-gray-600 bg-white p-2 ring-2 rounded-sm pl-5 pr-24"
                style={{
                  maxHeight: "120px",
                  overflowY: "auto",
                  minHeight: "50px",
              ...
              <div className="absolute right-3 bottom-2 flex space-x-2">
                <button
                  ...
                  onClick={() => {
                    if (textAreaInputRef.current && !loading) {
                      let modifiedPrompt = textAreaInputRef.current.value;
                      const dataSourceIds = Object.keys(selectedDataSources).filter(key => selectedDataSources[key] === true);
                      if (dataSourceIds.length > 0) {
                        console.log(dataSourceIds)
                        modifiedPrompt = modifiedPrompt + ` [DATA_SOURCES:${dataSourceIds.join(',')}]`
                      }
                      console.log(modifiedPrompt)
                      runWorkflow(modifiedPrompt);
                    }
                  }}
                  className="bg-accent hover:brightness-75 transition duration-300 rounded cursor-pointer flex justify-center items-center"
                ...
                <div className="relative">
                  <button
                    ...
                    className={`${isAnyDataSourceSelected ? 'bg-green' : 'bg-gray-200'} p-2 rounded-md bg-gray-200 hover:bg-gray-300 transition duration-300 rounded cursor-pointer flex justify-center items-center"`}
                    style={{ width: "35px", height: "35px" }}
                  >
                  ...
                  {showDropdown && (
                    <div className="absolute right-0 mt-2 w-48 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5">
                      <div className="py-1" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
                        {dataSourceOptions.map((dataSource: IDataSourceConfig) => (
                          <a
                            key={dataSource.id}
                            href="#"
                            ...
                            onClick={(e) => {
                              e.preventDefault();
                              handleDataSourceToggle(dataSource);
                            }}
                          >
                            <div className="flex items-center w-full">
                              ...
                              <span className="flex-grow">{dataSource.name}</span>
